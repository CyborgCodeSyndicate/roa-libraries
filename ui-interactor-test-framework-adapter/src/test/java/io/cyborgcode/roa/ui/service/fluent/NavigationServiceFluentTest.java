...
class NavigationServiceFluentTest extends BaseUnitUITest {
   ...
   void openNewTabShouldExecuteJavaScriptAndSwitchToNewTab() {
      // Create a SmartWebDriver mock
      SmartWebDriver jsDriver = mock(SmartWebDriver.class);

      // Create an original driver that implements JavascriptExecutor
      WebDriver originalDriver = mock(WebDriver.class, withSettings().extraInterfaces(JavascriptExecutor.class));
      JavascriptExecutor jsExecutor = (JavascriptExecutor) originalDriver;

      // Mock getOriginal() to return the original driver
      when(jsDriver.getOriginal()).thenReturn(originalDriver);
