package io.cyborgcode.roa.ui.playwright.util.table;
 ...
 *
 * <p>Supports operations like retrieving text values and locator elements from fields of type {@link TableCell}
...
public final class TableReflectionUtil {
   ...
   /**
    * Extracts all {@link Locator} instances from fields of type {@link TableCell} or {@code List<TableCell>}
    * within a given row object.
    *
    * @param row The row object containing {@link TableCell} fields.
    * @return A list of {@link Locator} objects extracted from the row.
    */
   public static List<Locator> extractElementsFromRow(Object row) {
      return Arrays.stream(row.getClass().getDeclaredFields())
            .filter(field -> TableCell.class.isAssignableFrom(field.getType()) || isListOfTableCell(field))
            .map(field -> {
               field.setAccessible(true);
               ...
               return Collections.<Locator>emptyList();
            })
            .flatMap(List::stream)
            .toList();
   }
