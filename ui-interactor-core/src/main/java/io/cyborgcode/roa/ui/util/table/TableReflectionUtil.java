package io.cyborgcode.roa.ui.util.table;
...
public final class TableReflectionUtil {
   ...
   /**
    * Extracts all {@link V} instances from fields of type {@link TableCell} or {@code List<TableCell>}
    * within a given row object.
    *
    * @param row The row object containing {@link TableCell} fields.
    * @return A list of {@link V} objects extracted from the row.
    */
   public static <V> List<V> extractElementsFromRow(Object row) {
      List<?> fields = Arrays.stream(row.getClass().getDeclaredFields())
            .filter(field -> TableCell.class.isAssignableFrom(field.getType()) || isListOfTableCell(field))
            .map(field -> {
               field.setAccessible(true);
               try {
