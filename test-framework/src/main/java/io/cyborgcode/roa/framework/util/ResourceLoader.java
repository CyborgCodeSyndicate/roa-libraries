...
public final class ResourceLoader {
      ...
      try (InputStream inputStream = getResourceAsStream(filePath)) {
         if (inputStream == null) {
            throw new MissingResourceException(
                  "File not found in resources: " + filePath, ResourceLoader.class.getName(),
                  filePath);
         }
         return new String(inputStream.readAllBytes(), StandardCharsets.UTF_8);
      } catch (IOException e) {
         throw new UncheckedIOException("Failed to load file from resources: " + filePath, e);
      }
   }
    ...
    *
    * <p>This method uses the class loader of the {@link Craftsman}
