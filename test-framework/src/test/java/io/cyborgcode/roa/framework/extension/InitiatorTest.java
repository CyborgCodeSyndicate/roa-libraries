...
import io.cyborgcode.roa.framework.extension.mock.MockInvocation;
import io.cyborgcode.roa.framework.extension.mock.MockLate;
import io.cyborgcode.roa.framework.extension.mock.MockPreQuestJourney;
import io.cyborgcode.roa.framework.extension.mock.MockTest;
        ...
        try (MockedStatic<SpringExtension> springExt = mockStatic(SpringExtension.class)) {
            ...
            try (MockedStatic<FrameworkConfigHolder> configMock = mockStatic(FrameworkConfigHolder.class)) {
                ...
                try (MockedStatic<ReflectionUtil> reflectionMock = mockStatic(ReflectionUtil.class)) {
                    reflectionMock.when(() ->
                                            ReflectionUtil.findEnumImplementationsOfInterface(eq(PreQuestJourney.class), eq("mockJourney"), eq(mockConfig.projectPackages()))
                    ).thenReturn(mockPreQuestJourney);
                    MockLate mockLate = new MockLate();
                    MockDataForge mockDataForge = new MockDataForge(mockLate);
                    reflectionMock.when(() ->
                                            ReflectionUtil.findEnumImplementationsOfInterface(eq(DataForge.class), eq("mockData"), eq(mockConfig.projectPackages()))
